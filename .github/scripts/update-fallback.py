...
def val_to_js(val, indent=8):
    ...
    if isinstance(val, str):
        return f'"{escape_js_string(val)}"'
    if isinstance(val, (int, float)):
        return str(val)
    if isinstance(val, dict):
        sp = ' ' * indent
        items = [f'{sp}  {k}: {val_to_js(v, indent+2)}' for k, v in val.items()]
        return '{\n' + ',\n'.join(items) + f'\n{sp}' + '}'
    if isinstance(val, list):
        sp = ' ' * indent
        items = [f'{sp}  {val_to_js(v, indent+2)}' for v in val]
        return '[\n' + ',\n'.join(items) + f'\n{sp}' + ']'
    return str(val)

def news_to_js(news_items):
    """Convert news array to JS source code as a plain array literal."""
    cards = []
